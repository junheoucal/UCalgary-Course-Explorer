...
const AddPrerequisite = () => {
  ...
  const handleRemovePrerequisite = async (requiredCourseID) => {
    try {
      await axios.delete(`http://localhost:8800/prerequisite/${CourseID}/${requiredCourseID}`);
      // Refresh prerequisites list
      const prereqRes = await axios.get(`http://localhost:8800/prerequisite/${CourseID}`);
      setPrerequisites(prereqRes.data);
      ...
      <div style={{ marginBottom: '40px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {prerequisites.map((prereq) => (
            ...
            >
              <span>{prereq.Required_CourseID} - {prereq.Course_Name}</span>
              <button 
                onClick={() => handleRemovePrerequisite(prereq.Required_CourseID)}
                style={{
                  padding: '5px 10px',
