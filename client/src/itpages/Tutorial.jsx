...
const Tutorial = () => {
    ...
    useEffect(() => {
        const fetchAllTutorial = async () => {
            try { 
                const res = await axios.get(`http://localhost:8800/tutorial/${CourseID}`);
    ...
    const handleDeleteTutorial = async (TutorialNo, semester_name) => {
        try {
            await axios.delete(`http://localhost:8800/tutorial/${CourseID}/${TutorialNo}/${semester_name}`);
        ...
        <div>
            ...
            <div className="tutorial">
                {tutorials.map((tutorial) => (
                    <div className="tutorial" key={`${tutorial.TutorialNo}-${tutorial.semester_name}`}>
                        ...
                        <button className="delete" onClick={() => handleDeleteTutorial(tutorial.TutorialNo, tutorial.semester_name)}>Delete</button>
                        <button className="update">
                            <Link to={`/itpages/UpdateTutorial/${CourseID}/${tutorial.TutorialNo}/${tutorial.semester_name}`}>Update</Link>
                        </button>
                    </div>
                ))}
            </div>
